...
*
* Name: Li-Ching, Cheng  Student ID: 143292175  Date: 2018/09/30
*
********************************************************************************/
/**********************************
 *          ALL DATA              *
...
var CustomerDB = {
  ...
  outputCustomerById: function (customer_id) {
    for (var i = 0; i < this.customers.length; i++) {
      if (customer_id == this.customers[i].customer_id) {
        ...
        var cust_add = this.getAddressById(this.customers[i].address_id);
        console.log('Home Address: ' + cust_add.address + ' ' + cust_add.city + ', ' + cust_add.province + '. ' + cust_add.postal_code);
  ...
  outputCustomersByStore: function (store_id) {
    for (var i = 0; i < this.stores.length; i++) {
      if (store_id == this.stores[i].store_id) {
        ...
        for (var l = 0; l < this.customers.length; l++) {
          if (store_id == this.customers[l].store_id) {
            this.outputCustomerById(this.customers[l].customer_id);
          }
        }
      }
    }
  ...
  removeCustomerById: function (customer_id) {
    for (var i = 0; i < this.customers.length; i++) {
      if (customer_id == this.customers[i].customer_id) {
        var remove = this.customers[i].address_id;
        this.customers.splice(i, 1);
        this.removeAddressById(remove);
      }
    }
  },
  removeAddressById: function (address_id) {
    ...
    for (var l = 0; l < this.stores.length; l++) {
      if (address_id == this.stores[l].address_id) {
        count++;
      }
    }
    if (count == 0) {
  ...
  outputAllAddresses: function () {
    ...
    for (var i = 0; i < this.addresses.length; i++)
    console.log('Address ' + this.addresses[i].address_id + ': ' + this.addresses[i].address
    + ' ' + this.addresses[i].city + ', ' + this.addresses[i].province
  ...
  outputAllStores: function () {
    ...
    for (var i = 0; i < this.stores.length; i++) {
      ...
      var loca = this.getAddressById(this.stores[i].address_id);
      console.log('Location: ' + loca.address + ' ' + loca.city + ', ' + loca.province
      + '. ' + loca.postal_code + '\n\n');
    }
  }
};
